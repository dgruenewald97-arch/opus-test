...
function renderProjects(){return `<section class="work-masthead"><h1>PROJECTS<sup>04</sup></h1><div class="work-lead"><p>Vier Marken.<br>Vier eigene Welten.</p><div><p>Von der ersten Idee bis zum Auftritt auf der Straße. Freie Markenstudien aus dem GRELLWERK-Konzept.</p><button type="button" data-reel-toggle aria-pressed="false">Bewegung pausieren</button></div></div><nav class="work-jump" aria-label="Projekt auswählen">${projects.map(p=>`<a href="#projekt-${p.id}">${p.name} ↘</a>`).join('')}</nav></section>
 <section class="work-editorial" aria-label="Unsere Projekte">${projects.map((p,i)=>`<article class="work-spread ${i%2?'is-reversed':''} work-${p.id}" id="projekt-${p.id}" style="--project-accent:${p.color}">
  ...
  <div class="work-composition"><a class="work-screen" data-work-screen href="case-${p.slug}.html" aria-label="${p.name}: Projekt ansehen">${motion[p.id]?loop(motion[p.id],{alt:''}):visual(p,{scene:true})}<div class="work-screen-shade"></div><span class="work-screen-line">${lines[p.id]}</span><h2${motion[p.id]?'':' class="sr-only"'}>${names[p.id]}</h2><span class="work-screen-open">DAS PROJEKT ↗</span></a>
  <div class="work-design"><a class="work-design-image" href="case-${p.slug}.html" aria-label="${p.name}: Gestaltung ansehen">${visual(p,{detail:i===1||i===3,scene:i===0})}<span aria-hidden="true">↗</span></a><h3>${p.line.replace(/\n/g,'<br>')}</h3><p>${p.intro}</p><a class="editorial-link" href="case-${p.slug}.html">Idee & Umsetzung ↗</a></div></div>
