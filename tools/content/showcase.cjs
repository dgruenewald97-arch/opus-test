...
function renderCase(p,i){const next=projects[(i+1)%projects.length];return `<section class="case-opening"><div class="case-opening-media">${visual(p,{scene:true,priority:true})}</div><div class="case-opening-shade"></div><a class="case-back" href="arbeiten.html">← Projects</a><div class="case-opening-title"><p>${p.discipline}</p><h1>${p.name}</h1><span>${context[p.id]}</span></div></section>
 <section class="case-narrative"><div><p>Die Idee</p><h2>${nl(p.line)}</h2></div><p>${p.intro}</p><div class="case-facts"><div><h3>Die Aufgabe</h3><p>${p.brief}</p></div><div><h3>Der Gedanke</h3><p>${p.idea}</p></div><div><h3>Die Umsetzung</h3><p>${p.output}</p></div></div></section>
${motion[p.id]?`<section class="case-motion" aria-label="${p.name}: Bewegungsstudie"><div class="case-motion-film">${loop(motion[p.id])}<div class="work-screen-shade"></div><p>${nl(p.poster)}</p></div><div class="case-motion-caption"><span>${p.name} / Bewegungsstudie</span><button type="button" data-reel-toggle aria-pressed="false">Bewegung pausieren</button></div></section>`:''}
 <section class="case-gallery-v4" aria-label="Kampagnenmotive"><figure data-reveal>${visual(p)}<figcaption>${p.name} / Die Marke</figcaption></figure><div class="case-type-poster" style="--poster-color:${p.color}" data-reveal><p>${nl(p.poster)}</p><span>${p.name}®</span></div><figure data-reveal>${visual(p,{detail:true})}<figcaption>${p.name} / Im Detail</figcaption></figure></section>
 <section class="case-team-v4"><h2>DAHINTER<br>STECKT TEAM.</h2><nav aria-label="Beteiligte Agenten">${['prompt','visual','voice','grid','syntax','cut','verify'].map(id=>{const a=agents.find(a=>a.id===id);return `<a href="ueber-uns.html?agent=${id}#crew"><strong>${a.name}</strong><span>${a.role}</span><span>↗</span></a>`;}).join('')}</nav><p class="fiction-note">Freie Konzeptarbeit · ${p.year} · Keine echte Kundenkampagne.</p></section>
