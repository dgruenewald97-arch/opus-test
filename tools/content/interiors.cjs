...
const {serviceOpening,journalOpening}=require('./experience.cjs');
...
function enhanceRetained(slug,html){
 if(slug==='leistungen'){
  html=html.replace(/<section\b[\s\S]*?<\/section>/,serviceOpening());
 ...
 if(slug==='journal')html=html.replace(/<section\b[\s\S]*?<\/section>/,journalOpening());
