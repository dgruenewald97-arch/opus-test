...
page('kontakt','Briefing vorbereiten','Deine Idee als lokales Briefing strukturieren, prüfen und kopieren. Ohne Versand.','contact',`
<section class="contact-head"><div class="page-kicker"><span>Briefing-Werkbank</span><span>Von der Idee zum nächsten Schritt.</span></div><h1>WAS HAST<br>DU VOR?</h1><div class="contact-content"><div><p>Eine Idee, die dich nicht loslässt?<br>Ein Problem, das nach Gestaltung ruft?<br>Bring es auf den Punkt.</p><a class="contact-mail" href="#f-msg">Mit deiner Idee beginnen ↘</a><p id="briefing-help" class="fiction-note">GRELLWERK ist ein fiktives Agenturkonzept. Hier bereitest du dein Briefing lokal vor: Idee beschreiben, Vorschau prüfen, Text kopieren. Es wird nichts versendet oder dauerhaft gespeichert. Persönliche Angaben sind freiwillig. Kopiere den Text, bevor du die Seite verlässt.</p><p class="briefing-intro" id="briefing-source" hidden></p></div><form id="contact-form" class="contact-form" aria-describedby="briefing-help"><label for="f-msg">Deine Projektidee <span>(Pflichtfeld)</span><textarea id="f-msg" name="message" rows="4" required placeholder="Was möchtest du verändern oder auf die Beine stellen?"></textarea></label><label for="f-goal">Was soll sich dadurch verändern? <span>(optional)</span><textarea id="f-goal" name="goal" rows="2" placeholder="Ziel, Zielgruppe oder gewünschtes Ergebnis"></textarea></label><label for="f-company">Marke / Unternehmen <span>(optional)</span><input id="f-company" name="company" autocomplete="organization" placeholder="Für wen wird’s laut?"></label><label for="f-name">Dein Name <span>(optional)</span><input id="f-name" name="name" autocomplete="name" placeholder="Dein Name"></label><label for="f-email">Deine E-Mail <span>(optional)</span><input id="f-email" name="email" type="email" autocomplete="email" placeholder="du@beispiel.de"></label><label for="briefing-preview">Dein Briefing · Vorschau<textarea class="briefing-preview" id="briefing-preview" rows="10" readonly spellcheck="false" aria-describedby="briefing-preview-help"></textarea></label><p id="briefing-preview-help" class="fiction-note">Die Vorschau aktualisiert sich beim Schreiben. Du kannst den Text auch direkt auswählen und kopieren.</p><div class="briefing-actions"><button class="pill" type="button" data-briefing-copy>Briefing kopieren <span aria-hidden="true">↗</span></button></div><p id="form-status" role="status" aria-live="polite" aria-atomic="true"></p><noscript><p>Für die automatische Vorschau brauchst du JavaScript. Deine Angaben werden nicht versendet.</p></noscript></form></div></section>`);
